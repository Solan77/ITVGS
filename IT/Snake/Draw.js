...
var drawModule = (function () {
    ...
    var bodySnake = function (x, y) {
        //Dette er den ene firkanten
        ctx.fillStyle = 'green';
        ctx.fillRect(x * snakSize, y * snakeSize, snakeSize, snakeSize);
        //dette er linjen rundt frikantenten
        ctx.strokeStyle = 'darkgreen';
        ctx.StrokeRect(x * snakSize, y * snakeSize, snakeSize, snakeSize);
    }
    var pizza = function (x, y) {
        //Dette er rundt pizza
        ctx.fillStyle = 'yellow';
        ctx.fillRect(x * snakeSize, y * snakeSize, snakeSize, snakeSize);
        //detter er den ene firkanten
        ctx.fillStyle = 'red';
        ctx.fillRect(x * snakeSize + 1, y * snakeSize + 1, snakeSize - 2, snakeSize - 2);
    }
    var scoreText = function () {
        //hvo rmange pizzaer har slangen spist
        var score_text = "Score: " + score;
        ctx.fillStyle = 'blue';
        ctx.fillText(score_text, 145, h - 5);
    }
    var drawSnake = function () {
        // Kroppen til slangen av 5 firkanter
        var length = 4;
        snake = [];

        // bruker en loopdytter vi 5 elementene inni firkantene
        // hvert element vil ha x = 0 og the y vil ta value index.
